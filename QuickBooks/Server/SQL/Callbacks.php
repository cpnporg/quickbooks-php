 ...
 * Static callback methods for the SQL mirror server
 */
class QuickBooks_Server_SQL_Callbacks
{
	/**
	...
	static public function onAuthenticate($requestID, $user, $hook, &$err, $hook_data, $callback_config)
	{
		...
		if ($mode == QUICKBOOKS_SERVER_SQL_MODE_READONLY or 
		...
		if ($mode == QUICKBOOKS_SERVER_SQL_MODE_WRITEONLY or 
			$mode == QUICKBOOKS_SERVER_SQL_MODE_READWRITE)
		{
...
if (!isset($table_and_field[0])) continue;				
	...
	public static function ListDeletedQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		if (!QuickBooks_Server_SQL_Callbacks::_requiredVersion(2.0, $version, $locale, QUICKBOOKS_DEL_LIST))
		{
	...
	public static function ListDeletedQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		foreach ($List->children() as $Node)
		{
				...
				$config['delete'] == QUICKBOOKS_SERVER_SQL_DELETE_FLAG)
			{
				...
				QuickBooks_Server_SQL_Callbacks::_DeleteChildren($table, $user, $action, $ID, $sqlObject, $extra, $config, true, true);
			}
			else
			{
	...
	public static function TxnDeletedQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		if (!QuickBooks_Server_SQL_Callbacks::_requiredVersion(2.0, $version, $locale, QUICKBOOKS_DELETE_TRANSACTION))
		{
			return QUICKBOOKS_SKIP;
		}
	...
	public static function ListDelRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		if ($Object = $Driver->get(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . strtolower($extra['objectType']), array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
			$xml .= '<?xml version="1.0" encoding="utf-8"?>
			<?qbxml version="' . QuickBooks_Server_SQL_Callbacks::_version($version, $locale) . '"?>
			<QBXML>
	...
	public static function ListDelResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		if ($errnum == QUICKBOOKS_XML_OK)
		{
				...
				$config['delete'] == QUICKBOOKS_SERVER_SQL_DELETE_FLAG)
			{
				...
				// Now Delete/Flag all the children
				QuickBooks_Server_SQL_Callbacks::_DeleteChildren($table, $user, $action, $ID, $object, $extra, $config, true, true);
			}
			else
			{
				// Otherwise we actually remove the rows
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Customer = $Driver->get(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'customer', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
			if(!(is_int($Customer->get('CreditCardInfo_ExpirationMonth')) && is_int($Customer->get('CreditCardInfo_ExpirationYear')) && checkdate($Customer->get('CreditCardInfo_ExpirationMonth'), 1, $Customer->get('CreditCardInfo_ExpirationYear')) ))
			{
				$Customer->remove('CreditCardInfo_ExpirationMonth');
				$Customer->remove('CreditCardInfo_ExpirationYear');
			}
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_CUSTOMER, $Customer, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function CustomerAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_CUSTOMER, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function ClassAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_CLASS, $Class, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function ClassAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_CLASS, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function DataExtAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		$xml = '';
		if ($DataExt = $Driver->get(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'dataext', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
			$xml .= '<?xml version="1.0" encoding="utf-8"?>
			<?qbxml version="' . QuickBooks_Server_SQL_Callbacks::_version($version, $locale) . '"?>
			<QBXML>
	...
	public static function DataExtAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_DATAEXT, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function ShipMethodAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_SHIPMETHOD, $Class, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function ShipMethodAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_SHIPMETHOD, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function PaymentMethodAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_PAYMENTMETHOD, $Class, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function PaymentMethodAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_PAYMENTMETHOD, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function AccountAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_ACCOUNT, $Account, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function AccountAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_ACCOUNT, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function ItemDiscountAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_DISCOUNTITEM, $Account, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function ItemDiscountAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_DISCOUNTITEM, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function ItemFixedAssetAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_FIXEDASSETITEM, $Account, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function ItemFixedAssetAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_FIXEDASSETITEM, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function ItemInventoryAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_INVENTORYITEM, $Account, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function ItemNonInventoryAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_NONINVENTORYITEM, $Account, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function ItemNonInventoryAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_NONINVENTORYITEM, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function ItemOtherChargeAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_OTHERCHARGEITEM, $Account, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function ItemOtherChargeAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_OTHERCHARGEITEM, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function ItemPaymentAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_PAYMENTITEM, $Account, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function ItemPaymentAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_PAYMENTITEM, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function ItemSalesTaxAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_SALESTAXITEM, $Account, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function ItemSalesTaxAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_SALESTAXITEM, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function ItemServiceAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_SERVICEITEM, $Account, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function ItemServiceAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_SERVICEITEM, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function ItemSubtotalAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_SUBTOTALITEM, $Account, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function ItemSubtotalAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_SUBTOTALITEM, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function EmployeeAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_EMPLOYEE, $Account, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function EmployeeAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_EMPLOYEE, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function EstimateAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_ESTIMATE, $Account, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function EstimateAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_ESTIMATE, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function PurchaseOrderAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_PURCHASEORDER, $Account, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function PurchaseOrderAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_PURCHASEORDER, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function ReceivePaymentAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_RECEIVEPAYMENT, $Account, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function ReceivePaymentAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_RECEIVEPAYMENT, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function ReceivePaymentModRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_RECEIVEPAYMENT, $ReceivePayment, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Account = $Driver->get(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'invoice', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_INVOICE, $Account, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function InvoiceAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_INVOICE, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function InvoiceModRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_INVOICE, $Invoice, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function JobTypeAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_JOBTYPE, $JobType, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function JobTypeAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_JOBTYPE, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function SalesOrderAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_SALESORDER, $Account, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function SalesOrderAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_SALESORDER, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function SalesRepAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_SALESREP, $Account, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function SalesRepAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_SALESREP, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function SalesTaxCodeAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_SALESTAXCODE, $Account, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function SalesTaxCodeAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_SALESTAXCODE, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function BillAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_BILL, $Customer, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function BillAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_BILL, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function BillModRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_ModRequest(QUICKBOOKS_OBJECT_BILL, $Bill, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function BillModResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsModResponse'] = true;
		return QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_BILL, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function BillPaymentCheckAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_BILLPAYMENTCHECK, $BillPaymentCheck, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function BillPaymentCheckAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_BILLPAYMENTCHECK, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function BillPaymentCreditCardAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_BILLPAYMENTCREDITCARD, $BillPaymentCreditCard, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function BillPaymentCreditCardAddResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		$extra['IsAddResponse'] = true;
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_BILLPAYMENTCREDITCARD, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function VendorAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_CUSTOMER, $Vendor, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function VendorModRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		if ($Vendor = $Driver->get(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'vendor', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
			return QuickBooks_Server_SQL_Callbacks::_AddRequest(QUICKBOOKS_OBJECT_VENDOR, $Vendor, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
		}
	...
	public static function VendorModResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_VENDOR, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	protected static function _AddRequest($type, $Object, $requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		//mail("grgisme@gmail.com", "April16PO", $action."\n\n".strtolower($type)."\n\n".var_export($daKids));
		
		
		if (!empty($daKids))//Get the rows
		{
	...
	{
		return QuickBooks_Server_SQL_Callbacks::_AddRequest($type, $Object, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config);
	}
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<AccountQueryRq requestID="' . $requestID . '">
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra) . '
					</AccountQueryRq>
				</QBXMLMsgsRq>
			</QBXML>';
	...
	public static function AccountQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('account', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	 ...
	 * 
	 */
	public static function BillPaymentCheckQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$xml = '';
		
		if (!QuickBooks_Server_SQL_Callbacks::_requiredVersion(2.0, $version, $locale, QUICKBOOKS_QUERY_BILLPAYMENTCHECK))
		{
			return QUICKBOOKS_SKIP;
		}
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<BillPaymentCheckQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
	 ...
	 * 
	 */
	public static function BillPaymentCheckQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		$Parser = new QuickBooks_XML_Parser($xml);
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('billpaymentcheck', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}

	public static function BillPaymentCreditCardQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$xml = '';
		
		if (!QuickBooks_Server_SQL_Callbacks::_requiredVersion(2.0, $version, $locale, QUICKBOOKS_QUERY_BILLPAYMENTCREDITCARD))
		{
			return QUICKBOOKS_SKIP;
		}
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<BillPaymentCreditCardQueryRq requestID="' . $requestID  . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
						<IncludeLineItems>true</IncludeLineItems>
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
	 ...
	 * 
	 */
	public static function BillPaymentCreditCardQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		$Parser = new QuickBooks_XML_Parser($xml);
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('billpaymentcreditcard', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function BillQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<BillQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra, $version, $locale) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
						<IncludeLineItems>true</IncludeLineItems>
	...
	public static function BillQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('bill', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	{
		if (!QuickBooks_Server_SQL_Callbacks::_requiredVersion(2.0, $version, $locale, QUICKBOOKS_QUERY_BILLTOPAY))
		{
			return QUICKBOOKS_SKIP;
		}
	...
	public static function BillToPayQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('billtopay', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function BillingRateQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		if (!QuickBooks_Server_SQL_Callbacks::_requiredVersion(6.0, $version, $locale, QUICKBOOKS_QUERY_BILLINGRATE))
		{
			return QUICKBOOKS_SKIP;
		}
	...
	public static function BillingRateQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('billingrate', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function BuildAssemblyQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		if (!QuickBooks_Server_SQL_Callbacks::_requiredVersion(5.0, $version, $locale, QUICKBOOKS_QUERY_BUILDASSEMBLY))
		{
			return QUICKBOOKS_SKIP;
		}
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<BuildAssemblyQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(5.0, $version, '<IncludeComponentLineItems>true</IncludeLineItems>') . '
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
	...
	public static function CheckQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<CheckQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
						<IncludeLineItems>true</IncludeLineItems>
						' . QuickBooks_Server_SQL_Callbacks::_requiredVersionForElement(7.0, $version, '<IncludeLinkedTxns>true</IncludeLinkedTxns>', $locale) . '
	...
	public static function ChargeQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<ChargeQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
						' . QuickBooks_Server_SQL_Callbacks::_requiredVersionForElement(3.0, $version, '<IncludeLinkedTxns>true</IncludeLinkedTxns>', $locale) . '
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>', $locale) . '
					</ChargeQueryRq>
				</QBXMLMsgsRq>
			</QBXML>';
	...
	public static function ChargeQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('charge', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	
	public static function ClassQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$xml = '';
	 ...
	 * 
	 */
	public static function ClassQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		$Parser = new QuickBooks_XML_Parser($xml);
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('class', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	
	
	public static function CompanyQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$xml = '';
	 ...
	 * 
	 */
	public static function CompanyQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		$Parser = new QuickBooks_XML_Parser($xml);
		...
		return QuickBooks_Server_SQL_Callbacks::_QueryResponse('company', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function CreditCardChargeQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<CreditCardChargeQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
						<IncludeLineItems>true</IncludeLineItems>
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
	...
	public static function CreditCardChargeQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('creditcardcharge', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function CreditCardCreditQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<CreditCardCreditQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
						<IncludeLineItems>true</IncludeLineItems>
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
	...
	public static function CreditCardCreditQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('creditcardcredit', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	
	
	public static function CreditMemoQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$xml = '';
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<CreditMemoQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
						<IncludeLineItems>true</IncludeLineItems>
						' . QuickBooks_Server_SQL_Callbacks::_requiredVersionForElement(1.1, $version, '<IncludeLinkedTxns>true</IncludeLinkedTxns>') . '
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
					</CreditMemoQueryRq>
				</QBXMLMsgsRq>
			</QBXML>';
	 ...
	 * 
	 */
	public static function CreditMemoQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		$Parser = new QuickBooks_XML_Parser($xml);
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('creditmemo', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function CustomerMsgQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		return QuickBooks_Server_SQL_Callbacks::_QueryResponse('customermsg', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function CustomerQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<CustomerQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra) . '
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
					</CustomerQueryRq>
				</QBXMLMsgsRq>
	...
	public static function CustomerQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		return QuickBooks_Server_SQL_Callbacks::_QueryResponse('customer', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	
	public static function CustomerTypeQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$xml = '';
	 ...
	 * 
	 */
	public static function CustomerTypeQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		$Parser = new QuickBooks_XML_Parser($xml);
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('customertype', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function DataExtDefQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		if (!QuickBooks_Server_SQL_Callbacks::_requiredVersion(2.0, $version, $locale, QUICKBOOKS_QUERY_DATAEXTDEF))
		{
			return QUICKBOOKS_SKIP;
		}		
	...
	public static function DataExtDefQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('DataExtDef', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function DateDrivenTermsQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('datedriventerms', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	 ...
	 * 
	 */
	public static function EmployeeQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$xml = '';
	 ...
	 * 
	 */
	public static function EmployeeQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		$Parser = new QuickBooks_XML_Parser($xml);
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('employee', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	 ...
	 * 
	 */
	public static function EstimateQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$xml = '';
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<EstimateQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
						<IncludeLineItems>true</IncludeLineItems>
						' . QuickBooks_Server_SQL_Callbacks::_requiredVersionForElement(3.0, $version, '<IncludeLinkedTxns>true</IncludeLinkedTxns>') . '
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
					</EstimateQueryRq>
				</QBXMLMsgsRq>
			</QBXML>';
	 ...
	 * 
	 */
	public static function EstimateQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		$Parser = new QuickBooks_XML_Parser($xml);
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('estimate', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function InventoryAdjustmentQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<InventoryAdjustmentQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
						<IncludeLineItems>true</IncludeLineItems>
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
	...
	public static function InventoryAdjustmentQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('inventoryadjustment', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function InvoiceQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<InvoiceQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
						<IncludeLineItems>true</IncludeLineItems>
						' . QuickBooks_Server_SQL_Callbacks::_requiredVersionForElement(1.1, $version, '<IncludeLinkedTxns>true</IncludeLinkedTxns>') . '
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
	...
	public static function InvoiceQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('invoice', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	
	public static function ItemServiceQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$xml = '';
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<ItemServiceQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra) . '
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
					</ItemServiceQueryRq>
				</QBXMLMsgsRq>
	...
	public static function ItemServiceQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		$Parser = new QuickBooks_XML_Parser($xml);
	...
	public static function ItemNonInventoryQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$xml = '';
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<ItemNonInventoryQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra) . '
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
	...
	public static function ItemNonInventoryQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		$Parser = new QuickBooks_XML_Parser($xml);
	...
	public static function ItemSalesTaxQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$xml = '';
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<ItemSalesTaxQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra) . '
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
					</ItemSalesTaxQueryRq>
				</QBXMLMsgsRq>
	...
	public static function ItemSalesTaxQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		$Parser = new QuickBooks_XML_Parser($xml);
	...
	public static function ItemQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<ItemQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra) . '
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
					</ItemQueryRq>
				</QBXMLMsgsRq>
	...
	public static function ItemQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse(QUICKBOOKS_OBJECT_ITEM, $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function ItemReceiptQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<ItemReceiptQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
						<IncludeLineItems>true</IncludeLineItems>
						<IncludeLinkedTxns>true</IncludeLinkedTxns>
					</ItemReceiptQueryRq>
	...
	public static function ItemReceiptQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('itemreceipt', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function JobTypeQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('jobtype', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	 ...
	 *
	 */
	public static function PaymentMethodQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$xml = '';
	 ...
	 * 
	 */
	public static function PaymentMethodQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		$Parser = new QuickBooks_XML_Parser($xml);
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('paymentmethod', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function PayrollItemWageQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('payrollitemwage', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function PriceLevelQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		if (!QuickBooks_Server_SQL_Callbacks::_requiredVersion(4.0, $version))
		{
			return QUICKBOOKS_SKIP;
		}
	...
	public static function PriceLevelQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('pricelevel', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function PurchaseOrderQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<PurchaseOrderQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
						<IncludeLineItems>true</IncludeLineItems>
						' . QuickBooks_Server_SQL_Callbacks::_requiredVersionForElement(3.0, $version, '<IncludeLinkedTxns>true</IncludeLinkedTxns>') . '
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
	...
	public static function PurchaseOrderQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('purchaseorder', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function ReceivePaymentQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		if (!QuickBooks_Server_SQL_Callbacks::_requiredVersion(1.1, $version))
		{
			return QUICKBOOKS_SKIP;
		}
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<ReceivePaymentQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
						<IncludeLineItems>true</IncludeLineItems>
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
	...
	public static function ReceivePaymentQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('receivepayment', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function SalesOrderQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		if (!QuickBooks_Server_SQL_Callbacks::_requiredVersion(2.1, $version))
		{
			return QUICKBOOKS_SKIP;
		}
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<SalesOrderQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
						<IncludeLineItems>true</IncludeLineItems>
						<IncludeLinkedTxns>true</IncludeLinkedTxns>
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
	...
	public static function SalesOrderQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('salesorder', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	
	
	
	
	public static function SalesReceiptQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$xml = '';
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<SalesReceiptQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
						<IncludeLineItems>true</IncludeLineItems>
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
					</SalesReceiptQueryRq>
	 ...
	 * 
	 */
	public static function SalesReceiptQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		$Parser = new QuickBooks_XML_Parser($xml);
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('salesreceipt', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function SalesRepQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('salesrep', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	
	
	
	public static function SalesTaxCodeQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$xml = '';
	 ...
	 * 
	 */
	public static function SalesTaxCodeQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		$Parser = new QuickBooks_XML_Parser($xml);
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('salestaxcode', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	
	
	
	public static function ShipMethodQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$xml = '';
	 ...
	 * 
	 */
	public static function ShipMethodQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		$Parser = new QuickBooks_XML_Parser($xml);
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('shipmethod', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	 ...
	 * 
	 */
	public static function TermsQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$xml = '';
	 ...
	 * 
	 */
	public static function TermsQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		$Parser = new QuickBooks_XML_Parser($xml);
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('terms', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function TimeTrackingQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<TimeTrackingQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
					</TimeTrackingQueryRq>
				</QBXMLMsgsRq>
			</QBXML>';
	...
	public static function TimeTrackingQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('timetracking', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function UnitOfMeasureSetQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		if (!QuickBooks_Server_SQL_Callbacks::_requiredVersion(7.0, $version))
		{
			return QUICKBOOKS_SKIP;
		}
	...
	public static function UnitOfMeasureSetQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('unitofmeasureset', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function VehicleMileageQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		if (!QuickBooks_Server_SQL_Callbacks::_requiredVersion(6.0, $version))
		{
			return QUICKBOOKS_SKIP;
		}
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<VehicleMileageQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
	...
	public static function VehicleMileageQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('vehiclemileage', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function VehicleQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('vehicle', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function VendorCreditQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<VendorCreditQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra, true) . '
						<IncludeLineItems>true</IncludeLineItems>
						' . QuickBooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<IncludeLinkedTxns>true</IncludeLinkedTxns>') . '
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
					</VendorCreditQueryRq>
				</QBXMLMsgsRq>
			</QBXML>';
	...
	public static function VendorCreditQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('vendorcredit', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	
	
	
	public static function VendorTypeQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$xml = '';
	 ...
	 * 
	 */
	public static function VendorTypeQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		$Parser = new QuickBooks_XML_Parser($xml);
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('vendortype', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	
	
	
	public static function VendorQueryRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$xml = '';
		...
		$xml .= '<?xml version="1.0" encoding="utf-8"?>
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<VendorQueryRq requestID="' . $requestID . '" ' . QuickBooks_Server_SQL_Callbacks::_buildIterator($extra) . '>
						' . QuickBooks_Server_SQL_Callbacks::_buildFilter($user, $action, $extra) . '
						' . Quickbooks_Server_SQL_Callbacks::_requiredVersionForElement(2.0, $version, '<OwnerID>0</OwnerID>') . '
					</VendorQueryRq>
				</QBXMLMsgsRq>
	 ...
	 * 
	 */
	public static function VendorQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		$Parser = new QuickBooks_XML_Parser($xml);
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('vendor', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	public static function WorkersCompCodeQueryResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() )
	{
		...
		QuickBooks_Server_SQL_Callbacks::_QueryResponse('workerscompcode', $List, $requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config);
	}
	...
	*/
		...
		*/
		...
		foreach($children as $child)
		{
			...
			$sql = "
				...
				WHERE 
					 " . $child['relField'] . " = '" . $keyID . "' 
				ORDER BY " . QUICKBOOKS_DRIVER_SQL_FIELD_ID . " ASC ";
			...
			while ($arr = $Driver->fetch($res))
			{
				...
				$retArr[$index] = array();
				$retArr[$index]["table"] = $table;
				$retArr[$index]["data"] = new QuickBooks_SQL_Object($table, null, $arr);
				if(!empty($child['children']))
					$retArr[$index]["children"] = QuickBooks_Server_SQL_Callbacks::_QueryChildren($child['children'], $arr[$child['children'][0]['parentKey']]);
				else
					$retArr[$index]["children"] = array();
			}
		}
	...
	protected static function _UpdateRelatives($table, $user, $action, $ID, $object, $extra, $callback_config = array(), $deleteDataExt = false, $fullDelete = false)
	{
		$update_relatives_map = array(
			...
			'account' => array( "id_field" => "ListID",
										"relatives" => array(
												"invoice" => "ARAccount_ListID"
												)
									),
									/*
					"bill" => array( "id_field" => "TxnID",
											"relatives" => array(
												"bill_expenseline" => "Bill_TxnID",
												"bill_itemgroupline" => "Bill_TxnID",
		...
		else
		{
			...
			$recordIdentifier = $object->get($update_relatives_map[$table]['id_field']);
			foreach($update_relatives_map[$table]['relatives'] as $key => $value)
			{
				
				$multipart = array( $value => $extra['AddResponse_OldKey'] );
				$tmpSQLObject = new QuickBooks_SQL_Object($key, null);
				
				//@todo Make the Boolean TRUE value used in the QUICKBOOKS_DRIVER_SQL_FIELD_DELETED_FLAG field a constant,
				//      in case the sql driver used uses something other than 1 and 0.
				$tmpSQLObject->set($value, $recordIdentifier);
				$Driver->update(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . $key, $tmpSQLObject, array( $multipart ), false);
			}
		}
	}
			...
			else
			{
				...
				foreach($delete_children_map[$table]['children'] as $key => $value)
				{
					...
					if ($fullDelete and
						...
						$callback_config['delete'] == QUICKBOOKS_SERVER_SQL_DELETE_FLAG)
					{
	...
	protected static function _QueryResponse($type, $List, $requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents, $callback_config = array())
	{
			...
			foreach ($objects as $key => $object)
			{
				...
				if (count($map) and $map[0] and $map[1])
				{
					...
					if ($tmp = $Driver->get(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . $table, $multipart ))
					{
						...
						if ($extra['AddResponse_OldKey'] == null and 
							Quickbooks_Utilities::keyForAction($action) == "TxnID")
						{
							$extra['AddResponse_OldKey'] = $tmp->get("TxnLineID");
						}
						
						if (isset($extra['IsAddResponse']) and 
						...
						else if (isset($extra['IsModResponse']) and 
							stripos($action, "dataext") === false)
						{
						...
						if (!isset($extra['IsAddResponse']) and
							$tmp->get(QUICKBOOKS_DRIVER_SQL_FIELD_MODIFY) > $tmp->get(QUICKBOOKS_DRIVER_SQL_FIELD_RESYNC) and
							$callback_config['mode'] != QUICKBOOKS_SERVER_SQL_MODE_READONLY)
						{
						...
						if ($actually_do_update)
						{
							...
							$hook_data = array(
								'hook' => QUICKBOOKS_SQL_HOOK_SQL_UPDATE, 
					...
					else
					{
						...
						if ($callback_config['mode'] != QUICKBOOKS_SERVER_SQL_MODE_WRITEONLY)
						{
							...
							$hook_data = array(
								'hook' => QUICKBOOKS_SQL_HOOK_SQL_INSERT, 
								'table' => QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . $table,
								'object' => $object,
								'data' => $object->asArray(), 
								'qbsql_id' => $last, 
		...
		if ($iterator_count !== false and 
			...
			( (int) $matches[1] ) > 0)
		{
			$extra = array( 'iteratorID' => $matches[2] ); // Set the iteratorID to be used
		...
		else
		{
			...
			$curr_sync_datetime = $Driver->configRead($user, $module, QuickBooks_Server_SQL_Callbacks::_keySyncCurr($action), $type, $opts);	// last sync started... 
	...
	protected static function _transformToSQLObjects($curpath, $Node, &$objects, $current = null, $extra = array())
	{
		if ($Node->childCount())
		{
			...
			foreach ($Node->children() as $Child)
			{
				...
				QuickBooks_Server_SQL_Callbacks::_transformToSQLObjects($curpath . ' ' . $Node->name(), $Child, $objects, null, $extra);
		...
		else
		{
			...
			if (isset($objects[$map[0]]))
			{
				...
				if ($map[0] == "dataextdef_assigntoobject")
				{
					...
					{
						$objects[$map[0]]->changePath(trim($curpath . ' ' . $Node->name()));
					}
				}
				else
				{
					if ($objects[$map[0]]->path() != trim($curpath) and
						strlen(trim($curpath)) < strlen($objects[$map[0]]->path()))
					{
						$objects[$map[0]]->changePath(trim($curpath));
					}
				}	
	...
	protected static function _buildIterator($extra, $version = null, $locale = null)
	{
		...
		if ($locale == QUICKBOOKS_LOCALE_CA)
		{
			return '';
		}
	...
	/*
	public function testGC($table)
	{
		$tArray = QuickBooks_Server_SQL_Callbacks::_GetChildrenTables($table);
		return $tArray;
	}
	*/
}
...
<InvoiceQueryRs requestID="SW52b2ljZVF1ZXJ5fGJlZmUwNWJlYjg0NDEyZWU1NjE0MGRhMzMwZjE0ZTdj" statusCode="0" statusSeverity="Info" statusMessage="Status OK" iteratorRemainingCount="0" iteratorID="{6fcbb18b-68cf-4adb-bf40-164ac0bb4ec0}">
	...
	<InvoiceRet>
		...
		<InvoiceLineRet>
			...
			<FullName>Test Service Item</FullName>
			</ItemRef>
...
QuickBooks_Server_SQL_Callbacks::AccountQueryResponse($requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $config = array() );
*/
...
QuickBooks_Server_SQL_Callbacks::DataExtDefQueryResponse($requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $config = array())
*/
...
//QuickBooks_Server_SQL_Callbacks::CustomerAddRequest($requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
...
<EditSequence>1249920532</EditSequence>
...
$tmp = QuickBooks_Driver_Singleton::getInstance('mysql://root:@localhost/accounting', array(), array(), QUICKBOOKS_LOG_DEVELOP);
print(QuickBooks_Server_SQL_Callbacks::InvoiceQueryResponse($requestID, $user, $action, $ID, $extra, $err, $last_action_time, $last_actionident_time, $xml, $idents, $callback_config));
*/
